#!/usr/bin/env python3
"""
Pre-build audit of the Chapter layer (deliverable A). Read-only.
"""
...
OUT = ROOT / "results/audit/chapter_audit.csv"
OUT_BOOKS = ROOT / "results/audit/chapter_books.csv"
OTHER = ("Sabche.yml", "Tsawa.yml", "Quotation.yml", "Citation.yml",
...
def main() -> int:
    ...
    print("=== COVERAGE ===")
    print(f"{'batch':6} {'books':>6} {'Chapter.yml':>11} {'non-empty':>10} {'spans':>8}")
    for bt in B:
    ...
    for pid, b in sorted(books.items()):
        ...
        for i, (s, e) in enumerate(sab):
            ...
            lens[bt].append(ln)
            seg = text[s:e]
            # fragmentation: next Chapter span separated only by punct/ws
            is_frag = False
            if i + 1 < len(sab):
                s2, e2 = sab[i + 1]
                if s2 >= e:
            ...
            for n, lst in others.items():
                if any(os_ < e and s < oe for os_, oe in lst):
                    ov_spans[bt][n] += 1
                    hit.append(n)
            if any(n in hit for n in ("Sabche.yml", "Tsawa.yml", "Quotation.yml", "Citation.yml")):
                ov_any[bt] += 1
            sy = syls(seg)
            if sy:
                first_syl[bt][sy[0]] += 1
        ...
        nch = len(text)
        book_rows.append({
            "pecha_id": pid, "batch": bt, "n_chapter": len(sab), "base_chars": nch,
            ...
            "n_sabche": len(others["Sabche.yml"]),
    ...
    print("\n=== CROSS-LAYER OVERLAP (% of Chapter spans touching layer) ===")
    print(f"{'layer':16} {'old':>8} {'new':>8}")
    for n in OTHER:
        print(f"{n.replace('.yml',''):16} " + " ".join(f"{pct(ov_spans[bt][n], adj[bt]):>8}" for bt in B))
    print(f"{'Sab|Tsa|Quo|Cit':16} " + " ".join(f"{pct(ov_any[bt], adj[bt]):>8}" for bt in B))
    ...
    print("\n=== PER-BOOK DENSITY (non-empty Chapter books) ===")
    for bt in B:
        br = [r for r in book_rows if r["batch"] == bt]
        cnt = [r["n_chapter"] for r in br]
    ...
    print(f"books w/ Tsawa or Commentary but NO Chapter: {len(noyml_comm)} "
          f"(old {sum(p.startswith('P') for p in noyml_comm)}, new {sum(p.startswith('I') for p in noyml_comm)})")

    # leakage
    print("\n=== LEAKAGE (Chapter spans >=40 chars, verbatim in another book) ===")
    ...
    n_rep = len(found)
    print(f"long Chapter spans: {n_long:,}; found verbatim in >=1 other book: {n_rep:,} ({pct(n_rep, n_long)})")
    for bt in B:
    ...
    # against split_v3
    chapter_in_split = [p for p in needles if p in split]
    print(f"Chapter books in split_v3: {len(chapter_in_split)}/{len(needles)} "
          f"(train {sum(split[p]['split']=='train' for p in chapter_in_split)}, "
          f"val {sum(split[p]['split']=='val' for p in chapter_in_split)}, "
          f"test {sum(split[p]['split']=='test' for p in chapter_in_split)})")
    train = {p for p, r in split.items() if r["split"] == "train"}
    for spn in ("val", "test"):
        nl = nh = 0
        for p in chapter_in_split:
            ...
            for i in range(len(needles[p])):
                ...
                if found.get((p, i), set()) & train:
                    nh += 1
        print(f"split_v3 leakage {spn}: {nh}/{nl} ({pct(nh, nl)}) of long Chapter spans appear in a train book")
    ...
    with (ROOT / "results/audit/chapter_links.csv").open("w", newline="", encoding="utf-8") as fh:
        w = csv.writer(fh); w.writerow(["a", "b", "share_a_in_b"])
        for (a, b) in sorted(links):
            w.writerow([a, b, round(share[(a, b)] / tot[a], 3)])
    print(f"\nwrote {OUT}, {OUT_BOOKS}, results/audit/chapter_links.csv")
    return 0
