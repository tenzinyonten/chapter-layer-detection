#!/usr/bin/env python3
"""
Build the Chapter BIO DatasetDict for mmBERT (no training).
...
def parse_args(argv=None):
    ...
    p.add_argument("--audit-csv", type=Path, default=ROOT / "data/tsawa_audit.csv")
    p.add_argument("--sidecar", type=Path, default=ROOT / "data/chapter_spans_clean.csv")
    p.add_argument("--split-file", type=Path, default=ROOT / "data/split_frozen.csv")
    p.add_argument("--tokenizer", default="jhu-clsp/mmBERT-base")
    p.add_argument("--max-length", type=int, default=8192)
    p.add_argument("--stride", type=int, default=5120)
    p.add_argument("--out-dir", type=Path, default=ROOT / "chapter_dataset")
    p.add_argument("--stats-json", type=Path, default=ROOT / "chapter_dataset_stats.json")
...
def main(argv=None) -> int:
    ...
    for r in csv.DictReader(args.sidecar.open(encoding="utf-8")):
        if r["dropped"] == "True" or r["pecha_id"] not in split:
            continue
        spans[r["pecha_id"]].append((int(r["start"]), int(r["end"]), r["ann_id"], "CHAPTER"))
    ids = sorted(split)
    if args.limit:
        ids = ids[: args.limit]
    ...
    for n, pid in enumerate(ids):
        ...
        n_b = sum(1 for x in labels if x == LABEL2ID["B-CHAPTER"])
        lost["spans"] += len(sp)
